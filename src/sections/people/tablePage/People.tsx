...
import { People as PeopleIcon, ArrowBack } from "@mui/icons-material";
...
export default function People() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const url = queryString ? `/api/people?${queryString}` : "/api/people";
  ...
  const handleRowClick = (person: PersonData) => {
    router.push(`/people/${encodeURIComponent(person.name)}`);
  };
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      {error && (
        <Alert severity="error" sx={{ mb: 2 }}>
          Error fetching people data: {error}
        </Alert>
      )}

      {!loading && !error && data.length === 0 && (
        <Alert severity="info">No people data available</Alert>
      )}
