...
import { StarshipData } from "./Starships.types";
import {
  DISPLAY_COLUMNS,
  SORT_OPTIONS,
  FILTER_OPTIONS,
} from "./Starships.constants";
...
export default function Starships() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to fetch starships data"
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleRowClick = (starship: StarshipData) => {
    router.push(`/starships/${encodeURIComponent(starship.name)}`);
  };
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      {error && (
        <Alert severity="error" sx={{ mb: 2 }}>
          Error fetching starships data: {error}
        </Alert>
      )}

      {!loading && !error && data.length === 0 && (
        <Alert severity="info">No starships data available</Alert>
      )}
